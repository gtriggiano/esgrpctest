...
function getEventsByAggregateTypesFactory(getConnection) {
  return function (_ref) {
    var aggregateTypes = _ref.aggregateTypes,
        fromEventId = _ref.fromEventId,
        limit = _ref.limit;

    var typesPlaceholders = aggregateTypes.map(function (_, i) {
      return '$' + (i + 2);
    }).join(', ');
    var queryStr = 'SELECT * FROM events\n                    WHERE id > $1\n                      AND aggregateType IN (' + typesPlaceholders + ')\n                    ORDER BY id ';
    var queryParams = [fromEventId].concat(_toConsumableArray(aggregateTypes));
...
exports.default = getEventsByAggregateTypesFactory;